...
class Package:
    ...
    def create_and_train_model(self):
        ...
        # joblib.dump(hist, 'backend/models/productModel.joblib')
        model.save('backend/models/productModel.h5')
        
    def evaluate(self):
        batch=self.get_dataset()
        # model = joblib.load('D:/Programing/Blockchain/Secure-DePaRT/backend/models/productModel.joblib')
        model = keras.models.load_model('backend/models/productModel.h5')
        # model=self.get_model()
        pre = Precision()
        re = Recall()
        acc = BinaryAccuracy()
    ...
    def predict(self):
        img = cv2.imread('backend/models/mobile3.png')
        print("IMG=",img)
        ...
        # with open('productModel_pkl' , 'rb') as f:
        #     model = pickle.load(f)
